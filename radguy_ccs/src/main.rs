...
macro_rules! weak_bisim_oracles {
    ($file_path:expr, $($name:ident: $left:expr, $right:expr => $ccs:expr;)*) => {
        ...
        weak_bisim_system_ordered! {
            ...
            BoolExtension::bitset().as_oracle().constant(StrategyWeight::Num(1)).then(SiblingsOracleInv),
            ...
            DependencyCountOracle::default().then(SiblingsOracle),
            DependencyCountOracle::default().then(SiblingsOracleInv),
            InverseDependencyCountOracle::default().then(StrategicArgumentsOracle::default()),
            InverseDependencyCountOracle::default().then(SiblingsOracle),
            InverseDependencyCountOracle::default().then(SiblingsOracleInv),
